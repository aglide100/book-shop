  ...
  public async insertOrderFromCart(
    ...
    try {
      ...
      let temp = await client.query(
        giveQ,
        [order.discount, order.member_no],
        (err, result) => {
          if (err) {
      ...
      let res = await client.query(orderQ, [
        order.order_no,
        order.price,
        order.member_no,
        order.credit_number,
      ...
      for (var i = 0; i < books.length; i++) {
        ...
        let response = await client.query(booksQ, [
          books[i].order_no,
          books[i].book_no,
        ...
        let bookMinusRes = await client.query(bookMiuns, [
          books[i].order_quantity,
          books[i].book_no,
        ]);
      ...
      if (cart_no != null) {
        ...
        let cartDeleteRes = await client.query(deleteQ, [cart_no]);
